...
RUN \
    wget -q -O /backend/deployment/cookiecutter-zope-instance.zip \
        "https://github.com/plone/cookiecutter-zope-instance/archive/refs/tags/${COOKIECUTTER_ZOPE_INSTANCE_VERSION}.zip" && \
    wget -q -O /backend/deployment/transform_from_environment.py \
    ...
    chmod u+x /backend/deployment/transform_from_environment.py && \
    chown -R plone:plone /backend/deployment
...
ENV ZOPE_TEMPLATE=/backend/deployment/cookiecutter-zope-instance.zip
ENV ZOPE_CONFIGURATION_FILE=/backend/deployment/instance-from-environment.yaml
...
ENTRYPOINT ["/backend/deployment/entrypoint.sh"]
